...
# Chunk frame-view rendering consumes saved result_pose.npz against decoded PLY
# geometry. Empirically this path aligns best when preserving the horizontal
# axis and only flipping the image up/down axis into OpenCV convention.
# This helper is not the authoritative mapping for docs/skills/sam3d-gso-render;
# that GSO render protocol intentionally keeps its own explicit axis-off bridge.
RENDER_POSE_TO_OPENCV = np.array([
    [1, 0, 0],
    [0, -1, 0],
    [0, 0, 1],
], dtype=np.float32)
...
def build_canonical_to_reference_cv_affine(
    scale: np.ndarray,
    rotation_quat: np.ndarray,
    translation: np.ndarray,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Build the affine transform that places canonical decoded geometry into the
    reference-camera OpenCV space described by ``result_pose.npz``.

    The decoded geometry is already in mesh [X, Y, Z] canonical layout, so this
    helper must not apply the latent [D, H, W] -> mesh [X, Y, Z] basis
    conversion again.

    Args:
        scale: Scalar or (3,) object scale in PyTorch3D camera space.
        rotation_quat: (4,) quaternion in wxyz order.
        translation: (3,) translation in PyTorch3D camera space.

    Returns:
        Tuple of (linear, offset), both float32.
    """
    scale_arr = _normalize_pose_vector(scale, expected=3, name="scale", allow_scalar=True)
    quat_arr = _normalize_pose_vector(rotation_quat, expected=4, name="rotation_quat", allow_scalar=False)
    translation_arr = _normalize_pose_vector(translation, expected=3, name="translation", allow_scalar=False)

    quat_tensor = torch.as_tensor(quat_arr, dtype=torch.float32).unsqueeze(0)
    rotation_matrix = quaternion_to_matrix(quat_tensor).squeeze(0).cpu().numpy()
    linear = RENDER_POSE_TO_OPENCV @ rotation_matrix @ np.diag(scale_arr)
    offset = RENDER_POSE_TO_OPENCV @ translation_arr
    return linear.astype(np.float32), offset.astype(np.float32)


def _normalize_pose_vector(
    value: np.ndarray,
    *,
    expected: int,
    name: str,
    ...
    if allow_scalar and array.size == 1 and expected == 3:
        return np.repeat(array, 3).astype(np.float32)
    if array.size != expected:
        raise ValueError(f"Expected {name} to contain {expected} values, got shape {np.asarray(value).shape}.")
