    ...
    def from_pretrained(cls, path: str, config_file: str = "pipeline.json") -> "Pipeline":
        ...
        for k, v in args['models'].items():
            if hasattr(cls, 'model_names_to_load') and k not in cls.model_names_to_load:
                continue
            if is_local:
                # Local pipeline configs may still reference remote repo ids.
                # Only join with `path` when local artifacts actually exist.
                local_ref = f"{path}/{v}"
                has_local_artifacts = (
                    os.path.exists(f"{local_ref}.json")
                    and os.path.exists(f"{local_ref}.safetensors")
                )
                model_ref = local_ref if has_local_artifacts else v
