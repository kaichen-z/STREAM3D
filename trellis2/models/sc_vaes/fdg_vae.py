...
def _find_ovoxel_package_dir() -> Path | None:
    for entry in sys.path:
        candidate = Path(entry) / "o_voxel" / "convert" / "flexible_dual_grid.py"
        if candidate.is_file():
            return candidate.parent.parent
    return None


def _load_flexible_dual_grid_to_mesh():
    try:
        from o_voxel.convert import flexible_dual_grid_to_mesh as fn
        return fn
    except ModuleNotFoundError as exc:
        # o_voxel.__init__ imports postprocess/rasterize which may depend on
        # optional CUDA extensions (flex_gemm/cumesh). We only need convert.
        if exc.name not in {"flex_gemm", "cumesh"}:
            raise

    package_dir = _find_ovoxel_package_dir()
    if package_dir is None:
        raise ModuleNotFoundError(
            "Cannot locate o_voxel convert module for TRELLIS2 mesh extraction."
        )
    ...
    if "o_voxel" not in sys.modules:
        ...
        pkg = sys.modules["o_voxel"]
        pkg.__path__ = [str(package_dir)]

    if "o_voxel.convert" not in sys.modules:
        convert_pkg = types.ModuleType("o_voxel.convert")
        convert_pkg.__path__ = [str(package_dir / "convert")]
        convert_pkg.__package__ = "o_voxel.convert"
        sys.modules["o_voxel.convert"] = convert_pkg

    module = importlib.import_module("o_voxel.convert.flexible_dual_grid")
    return module.flexible_dual_grid_to_mesh


flexible_dual_grid_to_mesh = _load_flexible_dual_grid_to_mesh()
